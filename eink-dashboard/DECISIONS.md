...
- **Bracket rendering (v3.15.11):** the shootout is shown **per team, vertically**, matching the regular score layout — each side shows its goals with the shootout total in parens (`Germany 1 (4)` / `Paraguay 1 (2)`), with the winner bold and "Penalties" on the when-line. (The first cut, v3.15.10, put a combined `"1-1 (4-2)"` on the when-line and suppressed the per-team score; the user preferred the vertical per-team format since the wide R32 boxes have the room.) The narrow inner-round boxes (R16→Final) have no per-team score column — a code/flag + inline score overflows them — so there the combined `scoreText` (`"1-1 (3-4)"`) stays on the when-line.
...
- Cache bumps: `wc:data:v3→v4` (new field on the blob), `wc:image:v10→v12` (mono pre-dithered image re-renders).
