...
**Fully data-driven, fills as it goes (v3.15.6):** the bracket renders EVERY round (R32→Final) from the live match list, not hardcoded placeholders. football-data publishes the R16/QF/SF/Final matches up front with empty teams + real dates; `bracketBox` shows just the round date (e.g. "Jul 4") as a roomy placeholder until a team is seeded, then renders the teams. So **how it updates over the tournament:** the page re-reads the source every ~12 min (SWR) + 6h cron; finished R32 ties flip to score + bold winner immediately, and R16+ boxes populate automatically as the source advances winners. R32 boxes were also slimmed (the first bracket made them too tall, squeezing the winner slots). **Open polish:** the bracket converges via box placement but does not yet draw the elbow connector *lines* between rounds.

### Known follow-up (not in this fix)

The R16 bracket layout clips its leftmost column on the mono panel (the "Round of 16" header and first ties are cut at the left edge). That only manifests in the `knockout` layout, which isn't reached until R16 (~2 weeks out), so it's deferred. Preview/verify with `?test-phase=knockout` before R16 begins.
