...
**Fix:** a dedicated `r32Layout` — the full 16-match round as a 2-column grid (`.wc-r32full`, 8 rows/column) filling the screen, sorted by kickoff, with scores for played ties, times for upcoming, and the ▶ favorite marker. No today/results split (it was redundant — the list already carries all of it). Color uses 19px + tighter cells so flagged rows like "Argentina v Cape Verde" fit without clipping; mono uses 20px (no flags). The `?test-phase=r32` fixture was expanded to a realistic 16-match round so this is verifiable without live data. Bumped `wc:image:v4 → v5` so any broken-layout PNG warmed between deploys is discarded.
