...
- **Knockouts (Round of 32 → Final)** — one **self-advancing bracket** for the whole knockout phase: the 16 R32 ties sit on the two outer edges and converge through the R16/QF/SF columns to a center Final box. R32 ties show both teams in full with a score (winner bold) or a date·time line; the narrower inner rounds show **3-letter codes on mono / flags on color**. Every round is computed from the previous round's winners, so a team that wins its tie appears in its next-round box immediately (the real source teams + score take over once that round is actually played); future rounds show their date until their teams are decided. The header names the **current** round (e.g. "Round of 16 · Jul 4 – 5 · times CT"). Ties decided on penalties show each team's goals with the shootout total in parens, vertically (**`Germany 1 (4)` / `Paraguay 1 (2)`**, winner bold, "Penalties" below), matching the regular score layout — at every round (the inner rounds use a tight `1(4)` form, with R32 narrowed to make room). (football-data folds the shootout into its full-time score; DECISIONS.md #50.)
...
- **B&W `/worldcup`** is served as a **server-pre-dithered 1-bit PNG**, not live HTML. SenseCraft renders pages in a cloud headless browser and dithers grayscale text edges into "fog" on the mono panel; to defeat that, the Worker uses **Cloudflare Browser Rendering** (`[browser]` binding + `@cloudflare/puppeteer`, `nodejs_compat`) to screenshot the Inter-styled HTML source (`?variant=src`) at 2×, supersample to 800×480, and threshold to pure black/white itself — so the cloud has no gray to fog and the panel shows crisp proportional type with full names. The image is cached `wc:image:v13` with stale-while-revalidate (14-min soft TTL) + a 6-h cron warm, so the device always gets an instant response despite the ~10 s browser launch; a cold cache falls back to the Inter HTML so the panel never blanks. Inter is inlined as base64 in `src/worldcup-fonts.ts`.
