...
- **KV operations are a tighter budget than neurons (DECISIONS.md #62)**: the free tier allows **1,000 writes/day** (and 1,000 lists) against 100,000 reads. A normal day already spends **~590 writes (~59%)**, mostly ephemeral weather/alert/device caches refreshed by two panels polling every 15 min — which is what triggers Cloudflare's "nearing the daily cap" emails. Cost any new per-request write path against ~400 spare writes/day. **Known latent fault:** the #58 stale-cache lookup does one KV `list` per request while AI is blocked (~40/hour of outage, ~960 over a full day vs a 1,000 cap), so a multi-day outage could exhaust the quota the fallback itself depends on. Not yet fixed — see #62.
