...
- **A panel request never waits on AI generation (v3.16.4)**: on a cold daily key with a healthy budget, `/fact.png`, `/fact1.png`, `/skyline.png` and `/color/moment` serve the most recent cached image immediately (`no-store`, `X-Stale-While-Generating: 1`) and generate today's in the background via `ctx.waitUntil`. SenseCraft's renderer gives up after ~8–10s and keeps a blank frame; a request-path generation takes 10–20s. Combined with the next-day pre-warm this is what keeps a page from going white (DECISIONS.md #64).
