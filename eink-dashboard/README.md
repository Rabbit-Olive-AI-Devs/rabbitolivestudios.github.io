...
| A page is **fully white** (no text, no alt text) on the panel | The renderer screenshotted before the page's `<img>` arrived — something on the request path took longer than SenseCraft's ~8–10s budget. Check `curl -w %{time_total}` on the image route and look for `X-Stale-While-Generating` in the headers; a cold key should answer with a stale image in <2s (DECISIONS.md #64). The device keeps the blank frame until a later render succeeds, so re-deploy the pagelist in SenseCraft HMI to force one. |
