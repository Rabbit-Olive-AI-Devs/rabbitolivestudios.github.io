...
| One page keeps regenerating instead of using the cron's cached image | The daily image warm must run *after* the Chicago date rolls over. `wrangler.toml` must schedule `5 5,6 * * *` so it fires at both 05:05 and 06:05 UTC (CDT and CST respectively) — otherwise the devices generate the images themselves on the request path and duplicate the work (DECISIONS.md #57). |
| `wrangler deploy` fails with `code: 10072` (5 cron triggers per account) | Workers Free caps cron triggers account-wide. Combine fire times into one expression (`5 5,6 * * *` rather than two entries). Note this failure still **uploads the Worker** — only the schedule update is rejected — so production ends up running new code on the old schedule. Verify with `GET /accounts/{id}/workers/scripts/{name}/schedules`. |
